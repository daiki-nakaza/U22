...
bool partsInfo::HitCheck() {		//地面との当たり判定　当たっていればtrue 当たっていなければfalse
	...
	if(g_MapChip[(y + IRONBALL_R  ) / MAP_SIZE + MapY][(x / MAP_SIZE + MapX)] != 1		//中心の真下
		|| g_MapChip[(y - IRONBALL_R) / MAP_SIZE + MapY][(x - IRONBALL_R / MAP_SIZE + MapX)] != 1	//左上
		|| g_MapChip[(y - IRONBALL_R) / MAP_SIZE + MapY][(x + IRONBALL_R / MAP_SIZE + MapX)] != 1	//右上
		|| g_MapChip[(y + IRONBALL_R) / MAP_SIZE + MapY][(x - IRONBALL_R / MAP_SIZE + MapX)] != 1	//左下
		|| g_MapChip[(y + IRONBALL_R) / MAP_SIZE + MapY][(x + IRONBALL_R / MAP_SIZE + MapX)] != 1	//右上
		){	
		return true;
	}
